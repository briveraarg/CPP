...
/*   Updated: 2025/12/05 18:59:17 by brivera          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main()
{
	std::cout << BOLD << BRIGHT_GREEN << "------ ex00 (pruebas) ------" << RESET << std::endl;
	...
	std::cout << BRIGHT_GREEN << "-- pruebas copia / asignación --" << RESET << std::endl;
	Bureaucrat copiaOli(Oli);
	std::cout << copiaOli << std::endl;
	Ana = Oli;
	std::cout << "Ana después de la asignación: " << Ana << std::endl;

	std::cout << BRIGHT_GREEN << "-- pruebas incremento / decremento --" << RESET << std::endl;
	Bureaucrat t1("Tester", 2);
	...
	try
	{
		...
		low.decrementGrade(); // debe lanzar
	}
	catch (const std::exception& e)
	{
		std::cerr << "capturada excepción: " << e.what() << std::endl;
